...
/**
 * Extract all searchable content from press release
 */
function extractSearchableContent(data) {
  const texts = [];

  if (data.title) texts.push(data.title);
  if (data.releaseLabel) texts.push(data.releaseLabel);
  if (data.location) texts.push(data.location);
...
module.exports = {
  ...
  async afterUpdate(event) {
    ...
    try {
      const fullDoc = await strapi.documents('api::press-release.press-release').findOne({
      ...
      if (fullDoc) {
        ...
        await strapi.db.query('api::press-release.press-release').update({
          where: { id: result.id },
